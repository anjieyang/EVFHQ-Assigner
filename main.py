...
def fetch_unassigned_video(conn):
    with conn.cursor() as cur:
        cur.execute(
            "SELECT video_id, resolution FROM videos WHERE location IS NULL AND server_id IS NULL LIMIT 1;")
        row = cur.fetchone()
        return row if row else None


def assign_task_to_downloader(video_id, resolution, channel, conn, server_id):
    with conn.cursor() as cur:
        message = json.dumps({
            'video_id': video_id,
        ...
        cur.execute(
            "UPDATE videos SET server_id = %s WHERE video_id = %s;", (server_id, video_id))
        conn.commit()
        print(f"Task for video_id {video_id} assigned to server {server_id}.")
...
def handle_task_request(channel, method, properties, body):
    ...
    result = fetch_unassigned_video(conn)
    if result:
        video_id, resolution = result
        assign_task_to_downloader(
            video_id, resolution, channel, conn, server_id)
    else:
...
def update_video_status(channel, method, properties, body):
    ...
    with conn.cursor() as cur:
        cur.execute(
            ...
            (data['location'], data['server_id'], data['video_id']) if data['status_code'] == 0 else ("FAILED", data['server_id'], data['video_id']))
        conn.commit()
        print(
    ...
    if result:
        video_id, resolution = result
        assign_task_to_downloader(
            video_id, resolution, channel, conn, data['server_id'])
    else:
        print("No videos left to process.")
    channel.basic_ack(delivery_tag=method.delivery_tag)
